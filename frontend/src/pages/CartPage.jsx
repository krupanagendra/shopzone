...
const CartPage = () => {
  ...
  const shippingPrice = itemsPrice > 100 ? 0 : (items.length > 0 ? 10 : 0);
        ...
        <div className="grid lg:grid-cols-3 gap-6">
            ...
            {items.map((item) => (
              <div key={item._id} className="bg-white rounded-lg shadow p-4 flex gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-amazon-yellow font-bold text-lg mt-1">${item.price.toFixed(2)}</p>
          ...
          <div className="bg-white rounded-lg shadow p-6 h-fit sticky top-24">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span>Items ({items.reduce((a,i)=>a+i.quantity,0)}):</span><span>${itemsPrice.toFixed(2)}</span></div>
              <div className="flex justify-between"><span>Shipping:</span><span className={shippingPrice === 0 ? "text-green-600" : ""}>{shippingPrice === 0 ? "FREE" : `$${shippingPrice.toFixed(2)}`}</span></div>
            ...
            {itemsPrice <= 100 && <p className="text-xs text-green-600 mt-2">Add ${(100-itemsPrice).toFixed(2)} more for FREE shipping!</p>}
